...
```
File installer akan dibuat di folder `packages/server/out/AudioMonitor_Server_Installer_v1.0.2.exe`.

### 3. Build Agent Installer Windows (`.exe`)
```bash
npm run build --workspace=packages/agent
```
File installer akan dibuat di folder `packages/agent/out/AudioMonitor_Agent_Installer_v1.0.2.exe`.
